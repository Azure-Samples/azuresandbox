...
4. **`terraform.tfvars` exists** in the repo root. If missing, run `./scripts/bootstrap.sh` (Linux Terraform execution environment) or `./scripts/bootstrap.ps1` (Windows). The two scripts are equivalent — pick the one matching the host OS. **How `bootstrap.sh` works (so you don't have to re-read it each run):**
   - **It is interactive** — it issues a series of `read -e` prompts and therefore must be run **attended (`/allow-all` mode off)**, batched with the other preflight prompts. It cannot complete unattended. Every prompt is pre-filled with a default: `arm_client_id` defaults from the `TF_VAR_arm_client_id` env var if it is exported (otherwise the prompt has no default), and all other prompts default from the signed-in `az` context. If `TF_VAR_arm_client_id` is not already set, surface `arm_client_id` via `ask_user` before running the script; if it is set, no `ask_user` prompt for it is needed.
...
**Execution-environment note (jumplinux2 only).** The `mode="async"` + `read_bash` pattern above applies to the **WSL / local** environment unchanged. On **`jumplinux2`** (reached via VS Code Remote-SSH), instead launch each long-running command (both `terraform apply` and `Invoke-UnitTests.ps1`) inside a **detached `tmux` session** so it survives Remote-SSH drops / VS Code server auto-shutdown, running outside the editor/agent process tree. Poll the tmux session's log on the same ~2-minute cadence to drive progress reports, and reattach to the session after any reconnect. All other rules — the 2-minute cadence, hung-process detection, `/allow-all` handoff, and error-handling policy — remain identical.
